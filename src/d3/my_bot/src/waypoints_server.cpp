...
private:
        ...
        if (warming > 2) {
            if (GridSpacePtr->getPoints().size() < maxNodes) { 
                ...
                if (nodes_built >= maxNodes) {
                    RCLCPP_INFO(this->get_logger(), "[Sim] PRM graph generation complete! System is ready for path requests.");
                } else {
                    RCLCPP_INFO(this->get_logger(), "[Init] Building PRM graph... (%ld/%d nodes)", nodes_built, maxNodes);
                }
            } 
    ...
    cv::Mat process_image(const sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        cv::Scalar lowerColor = cv::Scalar(100, 100, 100);  // Lower bound for shaded floor
        cv::Scalar upperColor = cv::Scalar(180, 180, 180);  // Upper bound for lit floor
        cv::Mat mask;
    ...
    void metadata_service_callback(
        ...
        if (path.valid) {
            RCLCPP_INFO(this->get_logger(), "[Sim] Valid path generated and sent to client.");
        } else {
            RCLCPP_WARN(this->get_logger(), "[Init] Requested path could not be found (or PRM graph is still generating).");
        }
