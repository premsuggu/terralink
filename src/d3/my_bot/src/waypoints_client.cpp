...
public:
    WaypointsClientNode() : Node("waypoints_client") {
        ...
        goal_pose_.x = -4.5; // Set goal pose {x: -4.5, y: 3.0, z: 0.0}
        goal_pose_.y = 3.0;
        goal_pose_.z = 0.0;

        // 10s timer for first service request
    ...
    void timer_callback() {
        ...
        if (!waypoints_) {
            ...
            while (!waypoints_client_->wait_for_service(std::chrono::seconds(1))) {
                if (!rclcpp::ok()) return;
                RCLCPP_INFO(this->get_logger(), "[Init] Waiting for waypoints_service to become available...");
            }
            
            RCLCPP_INFO(this->get_logger(), "[Init] Requesting path from waypoints_service...");
            ...
            auto response_received_callback = [this](ServiceResponseFuture future) {
                ...
                if (result->valid) {
                    waypoints_ = result;
                    RCLCPP_INFO(this->get_logger(), "[Sim] Received valid path! Beginning waypoint navigation.");
                } else {
                    RCLCPP_WARN(this->get_logger(), "[Init] Received invalid path. Retrying in 10s...");
                }
            };
