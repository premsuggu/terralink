...
class ElevationMappingNode(Node):
    ...
    def __init__(self):
        ...
        pc_topic = getattr(self.param, 'pointcloud_topic', '/camera/depth/points')
        
        # Subscriber: PointCloud2 - use explicit BEST_EFFORT QoS to match publisher
        ...
        # Publisher: GridMap - use RELIABLE QoS to match RViz GridMap display
        ...
        self.get_logger().info(f'Publish timer created with period {1.0 / self.param.publish_fps:.2f}s ({self.param.publish_fps} Hz)')
        self.get_logger().info(f'Timer object: {self.publish_timer}')
        self.get_logger().info(f'Timer callback: {self.publish_timer.callback}')
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        ...
        try:
            ...
            self.get_logger().info(f'Received pointcloud: {len(points_np)} points, frame: {msg.header.frame_id}')
            
            if len(points_np) == 0:
                self.get_logger().warn('Empty pointcloud received')
                return
            
            # Get transform: sensor_frame -> map_frame (sensor -> map)
            ...
            if transform is None:
                self.get_logger().warn('TF lookup failed, skipping pointcloud')
                return
            
            self.get_logger().info('TF lookup successful, fusing pointcloud')
            ...
            self.get_logger().info('Fusing pointcloud into elevation map')
            self.elevation_map.fuse_pointcloud(points_np, R, t)
            
            # Update traversability
            self.elevation_map.update_traversability()
            self.get_logger().info('Pointcloud fused successfully')
    ...
    def publish_map(self):
        ...
        self.get_logger().info('>>> publish_map ENTRY')
        self.get_logger().info('publish_map called')
        try:
            elev_layer = self.elevation_map.elevation_map[self.elevation_map.IDX_ELEVATION]
            self.get_logger().info(f'Elevation map stats: min={np.nanmin(elev_layer):.3f}, max={np.nanmax(elev_layer):.3f}, mean={np.nanmean(elev_layer):.3f}, valid={np.count_nonzero(~np.isnan(elev_layer))}')
