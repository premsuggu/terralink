#!/usr/bin/env python3
# Monkey patch distutils.msvccompiler for chainer on Linux
import sys
import types
if 'distutils.msvccompiler' not in sys.modules:
    m = types.ModuleType('distutils.msvccompiler')
    m.get_build_version = lambda: 0
    class DummyMSVCCompiler:
        def __init__(self, *args, **kwargs): pass
        def compile(self, *args, **kwargs): return []
        def create_static_lib(self, *args, **kwargs): pass
        def link(self, *args, **kwargs): pass
        def link_executable(self, *args, **kwargs): pass
        def link_shared_lib(self, *args, **kwargs): pass
        def link_shared_object(self, *args, **kwargs): pass
        def object_filenames(self, *args, **kwargs): return []
    m.MSVCCompiler = DummyMSVCCompiler
    m.__file__ = '<monkey-patch>'
    sys.modules['distutils.msvccompiler'] = m
