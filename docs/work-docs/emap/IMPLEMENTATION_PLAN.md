...
9. **GPU acceleration (CuPy)** — only after CPU correctness is locked in and CUDA/CuPy are installed and verified in this environment; port the fusion kernel, verify GPU output matches the CPU reference numerically.
10. **Drift compensation** — correct accumulated pose-drift error using low-variance cell statistics, unit tested against a synthetic drift scenario.
